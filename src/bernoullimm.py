...
import scipy.sparse
from _bernoullimm import confusion, labellogsumexp, labelresponsibilities
...
class BernoulliMM(BaseEstimator):
    ...
    def threshold_assignments(self,
                             responsibilities,
                             L,
                             n_labels,
                             threshold):
        """
        Get a mapping from labels to components by seeing
        which components have sufficient weight from examples of
        a given label above a threshold
        """
        labels_to_components = np.zeros((n_labels,self.n_components),dtype=self.float_type)
        for label in xrange(n_labels):
            labels_to_components[label] = responsibilities[L==label].sum(0) > threshold
    ...
    def component_logprobs(self, X):
        lpr = np.zeros((X.shape[0],self.n_components),
        ...
        if self.blocksize > 0:
            ...
            for block_id in xrange(0,X.shape[0],self.blocksize):
                ...
                lpr[block_id:blockend] = (log_product_of_bernoullis_mixture_likelihood(X[block_id:blockend], self.log_odds_,
    ...
    def label_threshold_eval(self, X,L,labels_to_components):
        """Evaluate the model on data where examples of particular
        labels are constrained to be to certain components
        ...
        if self.blocksize > 0:
            ...
            for block_id in xrange(0,X.shape[0],self.blocksize):
               ...
               + np.log(self.weights_))
                ...
                labellogsumexp(lpr,L[block_id:blockend],labels_to_components,logprob[block_id:blockend])
    ...
    def empirical_labels_to_components(self,responsibilities,L,components_to_labels,threshold):
        """Compute the confusion matrix using the responsibilities

        Parameters
        ----------
        responsibilities : array-like, shape= [n_samples, n_features]
        L : array-like, shape = [n_samples,]
        components_to_labels : array-like, shape = [n_components,]
        threshold : float
            Threshold for determining whether a cluster belongs to a particular label
        ...
        NCL : array-like, shape = [newn_components,], dtype=int
            NCL[i] is the label associated with new component i
        ...
        if splitmergetype=='all':
            # attempt to have one component for each value above threshold
            newn_components = np.sum(labels_components_weight > threshold)
            ...
            above_thresh_mask = labels_components_weight > threshold
            ...
            if len(split_candidate_cs) > 0:
                ...
                new_labels = np.nonzero(above_thresh_mask[:,split_c])
            else:
                split_c = -1
                num_new = 0

            if len(drop_candidate_cs) > 0:
                # drop the component imposing the largest loss
                drop_c = drop_candidate_cs[np.argmax((labels_components_weight.sum(0) - labels_components_weight.max(0))[drop_candidate_cs])]

            else:
                drop_c = -1

            newn_components = self.n_components + num_new - int(drop_c > -1)
            NCC= np.arange(newn_components)
            # drop the component first
            if drop_c > 0:
                NCC[drop_c:] += 1
        ...
        for new_c, (l, old_c) in enumerate(itertools.izip(NCL,
                                                          NCC)):
            if H[l,old_c] != -1:
                print "Already assigned new component to other label and old component"
                import pdb; pdb.set_trace()
            H[l,old_c] = new_c
    ...
    def fitthresholdlabels(self, X,L,threshold=100):
        """
        Run the EM algorithm to specified convergence.
        ...
        uniqueL = np.unique(L)
        n_labels = len(uniqueL)
        L = np.asarray(L,dtype=int)
        if not np.all(uniqueL == np.arange(n_labels)):
            unique_Ldict = dict( (v,i) for i,v in enumerate(uniqueL))
        ...
        for cur_init in range(self.n_init):
            ...
            for i in range(self.n_iter):
                # Expectation Step
                lpr = self.component_logprobs(X)
                curr_log_likelihood = logsumexp(lpr, axis=1).astype(self.float_type)
                responsibilities = np.exp(lpr - curr_log_likelihood[:, np.newaxis]).astype(self.float_type)
                labels_to_components = self.threshold_assignments(responsibilities,L,n_labels,threshold)

                curr_adj_log_likelihood = np.zeros(curr_log_likelihood.shape,dtype=self.float_type)
                adj_responsibilities = np.zeros((X.shape[0],self.n_components),dtype=self.float_type)
                ...
                if self.verbose:
                    print "Iteration {0}: loglikelihood {1} label_adj_loglikelihood {2}".format(i, log_likelihood[-1], adj_log_likelihood[-1])

                # check for convergence
                if i > 0 and abs(adj_log_likelihood[-1] - adj_log_likelihood[-2])/abs(adj_log_likelihood[-2]) < \
                   self.thresh:
                    self.converged_ = True
                    break
    ...
    def fitlabels(self, X,L,splitcriterion=np.inf):
        """
        Run the EM algorithm to specified convergence.
        ...
        L : array_like, shape (n_samples,)
            Vector indicating the label of each of the samples
            used for picking when to split or merge clusters
        ...
        # create a binary matrix representing the map between
        # the two
        ...
        if 'm' in self.init_params or not hasattr(self,'means_'):
            # if we have the label matrix the inititalization
            # is just the average over the datapoints within
            # a given label class
        ...
        if 'w' in self.init_params or not hasattr(self,'weights_'):
            self.weights_ = np.bincount(L)/n_samples
        ...
        for i in range(self.n_iter):
            # Expectation Step
            curr_log_likelihood, responsibilities = self.eval(X)
            confusion = self.predict_confusion(responsibilities,L,components_to_labels)
            # Label-adjusted expectation step
            splitmerge(responsibilities,components_to_labels)
            adj_log_likelihood, adj_responsibilities = self.evallabels(X,L,components_to_labels)
            # these are the log probabilities and responsibilities
            # if we constrain the data to fit within the components
            ...
            if self.verbose:
                ...
                print "Iteration {0}: loglikelihood {1} accuracy {2}".format(i, log_likelihood[-1],accuracy)
