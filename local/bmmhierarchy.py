...
np.save('%s/layer1_class_weights_9.npy' % expdir,bmm.weights_)
...
np.save('%s/layer1_class_responsibilities_9c.npy' % expdir,responsibilities.T)
...
for phone_id, phone in enumerate(leehon[:,0]):
    print phone_id
    for component_id in n_components:
        print component_id
        layer_n_components = min(int(phoneclass_bclass_counts[phone_id,component_id]/30),6)

        if layer_n_components == 0: continue

        bmm=bernoullimm.BernoulliMM(n_components=9,n_iter=100,n_init=5,verbose=True,float_type=np.float32,init_params='mw')
