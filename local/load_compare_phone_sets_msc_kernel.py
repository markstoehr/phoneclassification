...
def load_data_set(phn_set1,phn_set2, data_path, data_set_id):
    for phn_id,phn in enumerate(phn_set1):
        if phn_id == 0:
            X_data = np.loadtxt('%s/msc_features_17t_40f_%s_%d.dat' % (data_path,
                                                                   phn,data_set_id)
                                                                   )
            ...
            X = np.loadtxt('%s/msc_features_17t_40f_%s_%d.dat' % (data_path,
                                                                   phn,data_set_id)
                                                                   )
            while len(X) + n_data > X_data.shape[0]:
...
X_train, y_train = load_data_set(args.phn_set1,args.phn_set2, args.data_path, 0)
...
clf.fit(X,y)
X_test, y_test = load_data_set(args.phn_set1,args.phn_set2, args.data_path, 1)
s = clf.score(X_test,
                   y_test)
